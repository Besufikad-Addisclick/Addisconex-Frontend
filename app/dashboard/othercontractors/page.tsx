...
const SubcontractorCard = ({ contractor }: { contractor: Subcontractor }) => {
  ...
  if (!contractor.imageUrl) {
    console.warn(`Missing imageUrl for contractors: ${contractor.name} (ID: ${contractor.id})`);
  }
...
export default function OtherContractorsPage() {
  ...
  const fetchData = useCallback(
    debounce(async (filters: any, pageNum = 1) => {
      ...
      try {
        ...
        const response = await fetch(`/api/othercontractors/?${queryParams.toString()}`, {
          method: "GET",
          credentials: "include",
        });
    ...
    <div className="flex flex-col lg:flex-row gap-4 sm:gap-6 lg:gap-8">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex justify-between items-center mb-4 sm:mb-6">
          ...
          <div className="text-sm text-gray-600">
            {data.count > 0 && (
              <span>{data.subcontractors.length} of {data.count} contractors</span>
            )}
          </div>
        </div>
