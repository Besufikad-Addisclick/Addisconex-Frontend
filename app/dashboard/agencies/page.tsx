  ...
  return (
    <div className={`bg-white rounded-lg p-6 shadow-sm ${className}`}>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold">Filter Agencies</h2>
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <div className="flex-1 p-6">
          <div className="flex justify-between items-start">
            <div>
              <Link
                href={`/dashboard/agencies/${contractor.id}`}
  ...
  return (
    ...
    >
      ...
      <p className="text-gray-500 text-center max-w-md text-sm sm:text-base">
        We couldn&apos;t find any agencies matching your search criteria. Try
    ...
    debounce(async (filters: any, pageNum = 1) => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/agencies/?${queryParams.toString()}`,
          {
            method: "GET",
            credentials: "include",
          }
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.error || `Failed to fetch agencies: ${response.status}`
          );
        }
        ...
        console.log("Error fetching agencies:", err.message);
        setData({
          subcontractors: [],
          categories: [],
          regions: [],
