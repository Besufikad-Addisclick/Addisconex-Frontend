...
export default withAuth(
  async function middleware(req) {
    ...
    if (['/auth/login', '/auth/signup', '/subscription/success', '/subscription/failure', '/', '/pricing', '/service', '/contact', '/blog', '/products'].includes(pathname) || pathname.startsWith('/blog/')) {
    ...
    if (pathname.startsWith('/dashboard')) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.active_subscription && data.active_subscription.status === 'pending') {
  ...
  },
  {
    callbacks: {
      authorized: ({ token, req }) => {
        ...
        if (pathname.startsWith('/dashboard') || pathname === '/checkout' || pathname === '/choose-plan') {
          return !!token
        }
...
export const config = {
  matcher: ['/((?!api|_next/static|_next/image|favicon.ico|acx.png|santim.png|logotransparent.png|image.png).*)'],
}